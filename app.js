...
=======
// remove  var to const or let
//
var budgetController = (function () {
  // try don`t use this, because in moredtn programming it is not readeable
  var Expense = function (id, description, value) {
    this.id = id;
    this.description = description;
    this.value = value;
  ...
  //Don`t adress to prototype, because The most programmists don`t understand it and you can use another way to resolve problem
  Expense.prototype.calcPercentage = function (totalIncome) {
    if (totalIncome > 0) {
      this.percentage = Math.round((this.value / totalIncome) * 100);
    } else {
  ...
  return {
    ...
    calculateBudget: function () {
      ...
      if (data.totals.inc > 0) {
        // Calculate percentage of income that I spent
        data.percentage = Math.round((data.totals.exp / data.totals.inc) * 100);
      } else {
        data.percentage = -1;
      }
...
//Common problem: if you want to use OOP programmiring - use Typescript for this. Because there u can use standart class and after that compiler would compile this code to js with all prototypes etc.
...
var UIController = (function () {
  ...
  return {
    getInput: function () {
      return {
        // Method for returning the 3 inputs from the UI
        type: document.querySelector(DOMstrings.inputType).value,
        description: document.querySelector(DOMstrings.inputDescription).value,
        value: parseFloat(document.querySelector(DOMstrings.inputValue).value),
      };
    ...
    addListItem: function (obj, type) {
      ...
      if (type === "inc") {
        ...
        element = DOMstrings.incomeContainer;
        // don`t format html tags with string. Instead of create html in html page and then change data via JS
        html =
          '<div class="row item" id="inc-%id%"> <div class="col item__description">%description%</div> <div class="col text-right item__value">%value%</div> <div class="col-1 text-right item__delete"> <button class="btn item__delete-btn"> <i class="fas fa-trash"></i> </button> </div> </div>';
      } else if (type === "exp") {
        element = DOMstrings.expensesContainer;
        // don`t format html tags with string. Instead of create html in html page and then change data via JS
        html =
          '<div class="row item" id="exp-%id%"> <div class="col item__description">%description%</div> <div class="col text-right item__value">%value%</div> <div class="col-1 text-center item__percentage">21%</div> <div class="col-1 text-right item__delete"> <button class="btn item__delete-btn"> <i class="fas fa-trash"></i> </button> </div> </div>';
      }
...
/// main
  ...
  var ctrlAddItem = function () {
    ...
    if (input.description !== "" && !isNaN(input.value) && input.value > 0) {
      // Check if description is empty and the value is a number
      newItem = budgetCtrl.addItem(input.type, input.description, input.value); // 2. Add item to budgetCtrl
      UICtrl.addListItem(newItem, input.type); // 3. Add item to UICtrl
      UICtrl.clearFields(); // 4. Clear fields
      updateBudget(); // 5. Calculate and update budget
...
controller.init();
// use functions. Make all code separatly, don`t write it in one js file
